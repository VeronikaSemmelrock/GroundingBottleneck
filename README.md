...
The subdirectory `CAG` contains the adapted HCP encoding, including the filter conditions for constraint-aware guessing (CAG), as described in our paper.
...
ProASP is available on [GitHub](https://github.com/MazzottaG/ProASP). The version of ProASP used in our experiments was retrieved from GitHub on 16. December 2024. Note that ProASP is only supported on Linux.
...
After the installation, newground can be started with the command `newground {PATH_TO_PROBLEM_INSTANCE} > NEWGROUND_FILENAME.lp`. The file `NEWGROUND_FILENAME.lp` can then be grounded via gringo or solved via clingo (see above), providing the file as input. The `PATH_TO_PROBLEM_INSTANCE` file must contain the annotated HCP encoding for newground (`newground/HCP_encoding_annotated_newground.lp`) and the facts for the problem instance.
